...
function loguear(){
	. $DIRLOGBIN/Logep.sh "Install" "$1" "$2"
}
...
obtenerVariable(){
	echo $(grep $1 $GRUPO/$DIRCONF/Install.conf | cut -d '=' -f 2)
}

setearDireccionLoggerDefinitiva() {
	DIRLOGBIN=$GRUPO/$DIRBIN
}
